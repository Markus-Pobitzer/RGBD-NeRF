...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depth=False):
    ...
    depthfiles = []
    if load_depth:
        depthfiles = [f for f in imgfiles if "_depth." in f]
        imgfiles = [f for f in imgfiles if "_depth." not in f]

    if poses.shape[-1] != len(imgfiles):
        print('Mismatch between imgs {} and poses {} !!!!'.format(len(imgfiles), poses.shape[-1]))
        return

    if load_depth and len(imgfiles) != len(depthfiles):
        print('Mismatch between imgs {} and depth maps {} !!!!'.format(len(imgfiles), len(depthfiles)))
        return
    ...
    if load_depth:
        depths = [imread(f) for f in depthfiles]
        depths = np.stack(depths, -1)
        print('Loaded depth map', depths.shape)
        # print("Min depth:", min(depths[0]), "Max depth:", max(depths[0]))
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, load_depth=False):
    poses, bds, imgs, depths = _load_data(basedir, factor=factor,
                                          load_depth=load_depth)  # factor=8 downsamples original imgs by 8x
    ...
    if load_depth:
        depths = np.moveaxis(depths, -1, 0).astype(np.float32) * sc
